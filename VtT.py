...
def run_lora(args, clip_model_zs, logit_scale, test_loader):
    ...
    for idx, (t_all, x_all, y_all) in enumerate(test_loader):
        ...
        zs_acc = 0
        with torch.no_grad():
            full_texts = [format_prompt(args.prompt_template, classname) for classname in class_texts]
        ...
        fine_acc, acc_conbine, acc_mae, acc_both_mae = fsl_test(
            args,
            clip_model,
            query_images,
            query_label,
...
def fsl_test(args, clip_model, query_images, query_label, class_texts, mamba_net, supp_image, supp_label_noAug, emp_full_texts, give_text=None):
    clip_model.eval()
    with torch.no_grad():
        texts = class_texts
        with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
    ...
    with torch.no_grad():
        ...
        mae_comb_cosine_similarity = cosine_similarity * 0.5 + mae_supp_cosine_similarity * 0.5

        mae_supp_acc = cls_acc(mae_supp_cosine_similarity, target)
        mae_comb_acc = cls_acc(mae_comb_cosine_similarity, target)
